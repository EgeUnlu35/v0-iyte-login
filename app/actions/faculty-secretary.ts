...
/**
 * Get cover letters pending faculty secretary signature from department chairs
 * According to workflow: Status Required: COMPLETED, Stage Required: PENDING_FACULTY_SECRETARY
 */
export async function getCoverLettersForReview(): Promise<SuccessResponse<CoverLetterListResponse> | ErrorResponse> {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/faculty-secretary/cover-letters`, {
      method: "GET",
    ...
    if (result.data && result.data.coverLetters) {
      ...
      result.data.coverLetters = result.data.coverLetters.map((letter: any) => {
        ...
        return {
          ...
          notes: letter.notes || "",
          stage: coverLetterData.stage || letter.stage || "PENDING_FACULTY_SECRETARY",
          ...
          departmentChairSigned: Boolean(
            ...
            (coverLetterData.stage === "PENDING_FACULTY_SECRETARY" || coverLetterData.stage === "PENDING_STUDENT_AFFAIRS" || coverLetterData.stage === "FULLY_SIGNED") ||
            false
          ),
          ...
          studentAffairsSigned: Boolean(coverLetterData.studentAffairsSigned || letter.studentAffairsSigned),
      ...
      console.log("Faculty Secretary - Raw API data sample:", rawLetters[0]);
      console.log("Faculty Secretary - Nested structure analysis:", rawLetters.map((raw: any) => ({
        id: raw.id || raw.entryId,
        hasNestedCoverLetter: !!raw.coverLetter,
        topLevelStage: raw.stage,
        nestedStage: raw.coverLetter?.stage,
        topLevelDeptChairSigned: raw.departmentChairSigned,
        nestedDeptChairSigned: raw.coverLetter?.departmentChairSigned,
        nestedDeptChairSignedAt: raw.coverLetter?.departmentChairSignedAt,
      })));
      console.log("Faculty Secretary - All raw dept chair fields:", rawLetters.map((raw: any) => ({
        id: raw.id || raw.entryId,
        stage: raw.stage,
        departmentChairSigned: raw.departmentChairSigned,
        deptChairSigned: raw.deptChairSigned,
        departmentChairApproved: raw.departmentChairApproved,
        isSignedByDepartmentChair: raw.isSignedByDepartmentChair,
        departmentChairSignedAt: raw.departmentChairSignedAt,
        allFields: Object.keys(raw).filter(key => key.toLowerCase().includes('chair') || key.toLowerCase().includes('dept'))
      })));
      console.log("Faculty Secretary - Processed cover letters:", result.data.coverLetters.map((l: CoverLetter) => ({ 
        id: l.id, 
        stage: l.stage, 
        departmentChairSigned: l.departmentChairSigned,
        facultySecretary: l.facultySecretary,
        inferredFromStage: l.stage === "PENDING_FACULTY_SECRETARY" || l.stage === "PENDING_STUDENT_AFFAIRS" || l.stage === "FULLY_SIGNED"
      })))
    }
...
export async function getCoverLetterById(entryId: string): Promise<SuccessResponse<CoverLetter> | ErrorResponse> {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/faculty-secretary/cover-letters/${entryId}`, {
      method: "GET",
      headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
      cache: "no-store",
    })
...
/**
 * Sign cover letter as faculty secretary
 * According to workflow: Action: POST /api/faculty-secretary/cover-letters/{id}/sign
...
export async function signCoverLetter(
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/faculty-secretary/cover-letters/${entryId}/sign`, {
      method: "POST",
      headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
      cache: "no-store",
    })
