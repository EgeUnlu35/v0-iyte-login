...
export default function DepartmentSecretaryClientLayout({
  ...
  const [showApplicationCoverLetterModal, setShowApplicationCoverLetterModal] = useState(false)
  const [selectedApplicationForCoverLetter, setSelectedApplicationForCoverLetter] =
    useState<ApprovedApplication | null>(null)
  const [applicationCoverLetterNotes, setApplicationCoverLetterNotes] = useState("")
  const [isGeneratingApplicationCoverLetter, setIsGeneratingApplicationCoverLetter] = useState(false)
  ...
  const handleGenerateApplicationCoverLetter = async () => {
    ...
    try {
      ...
      const result = await generateApplicationCoverLetter(selectedApplicationForCoverLetter.id, payload)

      if (result.success) {
        alert(result.data.message || "Cover letter generated and sent to Department Chair for signing!")
        ...
        setActionError(result.error || "Failed to generate cover letter for application.")
      }
    } catch (error: any) {
      setActionError(error.message || "An unexpected error occurred while generating application cover letter.")
    } finally {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1 container mx-auto px-6 py-8 max-w-7xl">
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Manage graduation lists, student records, curriculum, applications, and departmental processes
          </p>
        </div>
                              ...
                              <td className="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
                                <div className="flex items-center gap-2 justify-end">
                                  <Button
                                    variant="outline"
                                    size="sm"
                                    onClick={() => handleOpenApplicationCoverLetterModal(app)}
          ...
          <TabsContent value="applications" className="space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  View and manage all student graduation applications and their statuses
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                    ...
                    <div key={app.id} className="border rounded-lg p-6 hover:shadow-md transition-shadow">
                      ...
                      <div className="flex flex-wrap gap-3 pt-4 border-t">
                        ...
                        <Button
                          ...
                          onClick={() => openReviewModal(app, "RETURNED_FOR_REVISION")}
                          disabled={isReviewing === app.id}
                        >
                  ...
                  {applications.length === 0 && (
                    <div className="text-center py-12">
                      ...
                      <p className="text-gray-500">No graduation applications have been submitted yet.</p>
                    </div>
                  )}
                </div>
              </CardContent>
          ...
          <TabsContent value="approved-apps" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Manage approved graduation applications. Generate cover letters and send them to Department Chair for
                  approval.
                </CardDescription>
              </CardHeader>
              <CardContent>
                ...
                >
                  <UserCheck className="w-4 h-4" />
                  {showApprovedApplicationsView ? "Hide" : "Load"} Approved Applications
                  {isLoadingApprovedApplications && <Loader2 className="ml-2 h-4 w-4 animate-spin" />}
                </Button>
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <DialogFooter>
              ...
              >
                ...
                Confirm{" "}
                {reviewAction === "APPROVED" ? "Approval" : reviewAction === "REJECTED" ? "Rejection" : "Return"}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      ...
      {selectedApplicationForCoverLetter && (
        <Dialog open={showApplicationCoverLetterModal} onOpenChange={setShowApplicationCoverLetterModal}>
          <DialogContent className="sm:max-w-[500px]">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
              ...
              <DialogDescription>
                For: {selectedApplicationForCoverLetter.studentName} {selectedApplicationForCoverLetter.studentLastName}{" "}
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input
                  id="appGpaModal"
                  value={selectedApplicationForCoverLetter.gpaOverall.toFixed(2)}
                  readOnly
                  className="col-span-3"
                />
              </div>
              ...
              <div className="grid grid-cols-4 items-start gap-4">
                <Label htmlFor="appCoverLetterNotesModal" className="text-right pt-1">
                  Notes
                </Label>
                <Textarea
                  id="appCoverLetterNotesModal"
                  value={applicationCoverLetterNotes}
                  onChange={(e) => setApplicationCoverLetterNotes(e.target.value)}
                  className="col-span-3"
                  placeholder="Add any additional comments or notes for the cover letter..."
                />
              </div>
            ...
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setShowApplicationCoverLetterModal(false)}>
                Cancel
              </Button>
              <Button onClick={handleGenerateApplicationCoverLetter} disabled={isGeneratingApplicationCoverLetter}>
