...
export default function StudentClientLayout({
    ...
    const loadStoredData = async () => {
      ...
      if (storedTranscript) {
        ...
        setTotalCreditsForApplicationForm(
          storedTranscript.total_credit.toString()
        );
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        if (result.data.valid) {
          alert(
            "Transcript validated successfully! You can now fill the application form."
          );
          setCurrentStep(ApplicationStep.FillApplication);
        } else {
          alert(
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      ...
      APPROVED_BY_DEPSEC: {
        color: "bg-green-100 text-green-800",
        text: "Approved by Department Secretary",
      },
              ...
              {application.missingCourses0 && (
                <div className="p-3 bg-orange-50 rounded-lg">
                  ...
                  <ul className="text-sm list-disc list-inside">
                    {application.missingCourses0 && (
                      <li>{application.missingCourses0}</li>
                    )}
                    {application.missingCourses1 && (
                      <li>{application.missingCourses1}</li>
                    )}
                    {application.missingCourses2 && (
                      <li>{application.missingCourses2}</li>
                    )}
                  </ul>
                </div>
              )}
          ...
          (!application || application.status === "REJECTED") && (
            <Card className="mb-6">
              <CardHeader>
                ...
                <CardDescription>
                  Complete the steps below to submit your graduation
                  application.
                </CardDescription>
              </CardHeader>
              <CardContent>
              ...
              {currentStep === ApplicationStep.UploadTranscript && (
                <div className="space-y-4 p-4 border rounded-md">
                  ...
                  <p className="text-sm text-gray-600">
                    Upload your official transcript PDF. The system will analyze
                    it and extract your GPA and credit information, then compare
                    it with the curriculum requirements.
                  </p>
                  <div>
              ...
              {currentStep === ApplicationStep.ValidateDocuments && (
                <div className="space-y-4 p-4 border rounded-md">
                  ...
                  <Alert className="bg-green-50 border-green-200 text-green-700">
                    ...
                    <AlertDescription>
                      Transcript uploaded successfully! (GPA:{" "}
                      {transcriptUploadData?.agno}, Credits:{" "}
                      {transcriptUploadData?.total_credit})
                    </AlertDescription>
                  </Alert>
                  <p className="text-sm text-gray-600">
                    Now we'll validate your transcript against the curriculum
                    requirements to check if you have completed all necessary
                    courses.
                  </p>
                  <Button
                ...
                validationResult?.valid && (
                  ...
                  >
                    ...
                    <Alert className="bg-green-50 border-green-200 text-green-700">
                      <CheckCircle className="h-4 w-4" />
                      <AlertDescription>
                        Transcript validated successfully! Please review the
                        details below.
                      </AlertDescription>
                    </Alert>
